...
parser.add_argument("--input_image", type=str, default='./data/%s/' % (ROOT_DIR),
...
with tf.Session() as sess:
    ...
    for img_name in test_img_files:
        ...
        if not scores_.any():  # detect no flaw
            cv2.imwrite('./%s/dp-qual/' % (RESULT_DIR) + img_name, img_ori)
        else:  # deep unqual
            cv2.imwrite('./%s/dp-unqual/' % (RESULT_DIR) + img_name, img_ori)
        # cv2.waitKey(0)

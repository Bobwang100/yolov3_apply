...
CHECK_DIR = 'tile_checkpoint_56'
### Some paths
train_file = './data/%s/train.txt' % ROOT_DIR # The path of the training txt file.
val_file = './data/%s/val.txt' % ROOT_DIR  # The path of the validation txt file.
# restore_path = './data/darknet_weights/yolov3.ckpt'  # The path of the weights to restore.
restore_path = './tile_checkpoint_n8/model-epoch_290_step_215048_loss_0.0199_lr_1e-05'  # The path of the weights to restore.
save_dir = './%s/' % CHECK_DIR  # The directory of the weights to save.
log_dir = './data/tile_logs/'  # The directory to store the tensorboard log files.
progress_log_path = './data/tile_progress.log'  # The path to record the training progress.
anchor_path = './data/yolo_anchors.txt'  # The path of the anchor txt file.
class_name_path = './data/tile_flaw.names'  # The path of the class names.
...
val_evaluation_epoch = 2  # Evaluate on the whole validation dataset after some epochs. Set to None to evaluate every epoch.2
...
warm_up_epoch = 3  # Warm up training epoches. Set to a larger value if gradient explodes. 3
